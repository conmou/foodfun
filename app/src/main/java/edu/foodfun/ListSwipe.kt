...
class ListSwipe(private val context: Context, private var listSwipeType: ListSwipeType) : ItemTouchHelper.Callback() {
    ...
    fun setSwipeType(listSwipeType: ListSwipeType){
        ...
        if (listSwipeType == ListSwipeType.WILLEAT) {
            limitScrollX = dipToPx(180f, context)
        }
        else if (listSwipeType == ListSwipeType.FAVORITE) {
            limitScrollX = dipToPx(90f, context)
        }
    }
